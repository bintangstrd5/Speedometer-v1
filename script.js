document.addEventListener('DOMContentLoaded', () => {
  ...
  window.setVehicleType = (type) => {
    ...
    if (els.icons.seatbelt) {
      if (vehicleState.isMotorcycle) {
        ...
        manageLoopingAudio(els.audio.alarm, false); // Matikan alarm saat ganti ke motor
      } else {
  ...
  window.setFuel = (val) => {
    ...
    if (els.fuelPercent) els.fuelPercent.textContent = Math.round(p * 100) + '%';
  };
  ...
  window.setHealth = (val) => {
    ...
    if (els.healthPercent) els.healthPercent.textContent = Math.round(p * 100) + '%';
  };
  ...
  window.setEngine = (on) => {
    ...
    if (!newState) {
      ...
      // Jika motor, pastikan alarm tidak menyala
      if (vehicleState.isMotorcycle) {
        manageLoopingAudio(els.audio.alarm, false);
        return;
      }
